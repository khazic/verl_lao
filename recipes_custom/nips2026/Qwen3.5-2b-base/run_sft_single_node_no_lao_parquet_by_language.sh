...
files = sorted(
    ...
    if not os.path.basename(f).startswith("lao__to__")
    and "__to__lao.parquet" not in os.path.basename(f)
)
if not files:
    raise SystemExit(f"No non-lao parquet files found under {parquet_dir}")
print("[" + ",".join(files) + "]")
PY
)}

PROJECT_NAME=${PROJECT_NAME:-nips2026_qwen3_5_2b_base}
EXPERIMENT_NAME=${EXPERIMENT_NAME:-no_lao_parquet_by_language}
