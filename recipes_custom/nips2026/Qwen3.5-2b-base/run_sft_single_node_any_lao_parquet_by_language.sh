...
src_lao = glob.glob(os.path.join(parquet_dir, "lao__to__*.parquet"))
tgt_lao = glob.glob(os.path.join(parquet_dir, "*__to__lao.parquet"))
files = sorted(set(src_lao + tgt_lao))
if not files:
    raise SystemExit(f"No lao-related parquet files found under {parquet_dir}")
print("[" + ",".join(files) + "]")
PY
)}
